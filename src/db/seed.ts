...
async function seedRooms() {
    for (let i = 0; i < 50; i++) {
        ...
        try {
            const result = await queryDb('INSERT INTO rooms SET ?', room);
            console.log(result);
        }
        catch(err) {
            console.error(err);
...
async function seedBookings() {
    for (let i = 0; i < 50; i++) {
        const booking = {
            ...
            room_number: faker.datatype.number(50),
            price: Number(faker.commerce.price(50)),
            special_request: faker.lorem.sentence(),
        };
        try {
            const result = await queryDb('INSERT INTO bookings SET ?', booking);
            console.log(result);
        }
        catch(err) {
            console.error(err);
...
async function seedUsers() {
    for (let i = 0; i < 50; i++) {
        ...
        try {
            const result = await queryDb('INSERT INTO users SET ?', user);
            console.log(result);
        }
        catch(err) {
            console.error(err);
...
async function seedContact() {
    for (let i = 0; i < 50; i++) {
        ...
        try {
            const result = await queryDb('INSERT INTO contact SET ?', contact);
            console.log(result);
        }
        catch(err) {
            console.error(err);
